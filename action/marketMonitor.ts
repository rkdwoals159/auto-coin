...
import { createGateIOMarketBuyOrderByPercentage, getGateIOContractInfo } from '../gateio/request/post/createFuturesOrder';
...
export class MarketMonitor {
    ...
    async startMonitoring(
        ...
        while (this.isRunning && new Date() < endTime) {
            try {
                ...
                if (shouldPause) {
                    Logger.logThresholdExceeded(pauseThreshold);
                    ...
                    if (highestDifference && orderlyAccountId && orderlyApiKey && orderlySecretKey) {
                        ...
                        Logger.logNoAuthWarning('Orderly', ['ORDERLY_ACCOUNT_ID', 'ORDERLY_API_KEY', 'ORDERLY_SECRET_KEY']);
                    }
                }
                ...
                if (this.priceMonitor.getTotalExecutions() % 60 === 0) {
                    ...
                    Logger.logProgress(elapsedMinutes, this.priceMonitor.getTotalExecutions());
                }
        ...
        Logger.logMonitoringComplete(finalEndTime, this.priceMonitor.getTotalExecutions());
            ...
            if (existingPosition) {
                ...
                if (gateioQuantity) {
                    ...
                    if (parallelResult.success) {
                        Logger.logParallelOrderSuccess(
                            String(parallelResult.orderly?.order_id || 'N/A'),
                            String(parallelResult.gateio?.id || 'N/A')
                        );
                        ...
                        if (newPosition) {
                            ...
                            this.positionManager.setPositionEntryPrice(
                                ...
                                orderlyEntryPrice,
                                gateioEntryPrice,
                                Math.abs(newPosition.position_qty)
                            );

                            Logger.logArbitrageEntry(
                                coinSymbol,
                                orderlyEntryPrice,
                                gateioEntryPrice,
                                Math.abs(newPosition.position_qty),
                                'Orderly 매수 + Gate.io 매도'
                            );
                        ...
                        Logger.logParallelOrderFailure(String(parallelResult.error || '알 수 없는 오류'));
                    }
                } else {
    ...
    printMonitoringResult(result: MarketMonitoringResult): void {
        Logger.logMonitoringResult(result);
    }
}